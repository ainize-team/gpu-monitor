...
def _make_message(
    ...
    if status == 'success':
        ...
        fields = [
            {
                "value": f"First Occurrence Time: {first_occurrence_time}",
                "short": False
            },
            {
                "value": f"GPU Utilization: {utilization}",
                "short": False
            }
        ]
    return [
...
class SlackWebhookBot:
    ...
    def send_message(self, status: str, server_name: str, first_occurrence_time: str, utilization: float) -> bool:
        try:
            response = requests.post(
                url=self.webhook_url,
                headers={'Content-Type': 'application/json; charset=utf-8'},
                json={"attachments": _make_message(status, server_name, first_occurrence_time, utilization)}
            )
